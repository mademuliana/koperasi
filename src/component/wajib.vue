<template>
  <div>
      <div id="content">
        ...
        <b-modal id="bv-modal-example-wajib" hide-footer title="Data setoran">
          ...
          <div class="d-block">
            <form @submit.prevent="add">
              ...
              <input type="hidden" v-model="form.jenis_setoran" value="wajib">
         ...
         <b-modal id="bv-modal-example-wajib-update" hide-footer title="Data setoran">
          <p class="">tolong isi semua data dibawah ini dengan sebenar-benarnya</p>
          <div class="d-block">
            <form @submit.prevent="add">
              <input type="hidden" v-model="form.id">
         ...
         <div class="row"  style="padding-bottom:10px" >
           <div class="col" style="padding-left:30px">
             <div class="row">
              <div class="col-xs-2">
                <button class="btn btn-primary right button" id="show-btn" @click="$bvModal.show('bv-modal-example-wajib')"><font-awesome-icon icon="plus" /> Tambah</button>
              </div>
             </div>
           </div>
        ...
        <table class="table table-bordered t-width" style="table-layout: fixed">
            ...
            <tbody>
                <tr  v-for="wajib in sortedWajib" :key="wajib.id"  >
                    <td>{{wajib.kta}}</td>
                    <td>{{wajib.penyetor}}</td>
                    <td>{{wajib.tanggal_setoran}}</td>
                    <td>{{wajib.jumlah_setoran}}</td>
                    <td></td>
                    <td><center><p><font-awesome-icon class="link-t" id="show-btn" @click="($bvModal.show('bv-modal-example-wajib-update')),(edit(user))" icon="pen" /><font-awesome-icon class="link-d" id="show-btn" @click="del(user)" icon="trash-alt" /></p></center></td>
...
export default {
  ...
  methods: {
    ...
    loadSimpanan(){
        axios.get('http://localhost:3000/wajib').then(res => {
      ...
      add(){
      axios.post('http://localhost:3000/wajib', this.form).then(res => {
          this.load()
          this.form.penyetor = '',
          this.form.kta='',
          this.jenis_setoran='',
    ...
    update(form){ 
       return axios.put('http://localhost:3000/wajib' + form.id , {
    ...
    del(user){
      axios.delete('http://localhost:3000/wajib' + user.id).then(res =>{
          this.load()
