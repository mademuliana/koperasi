  ...
  <div>
      <div id="content">
        ...
        <table class="table table-bordered t-width" style="table-layout: fixed">
            ...
            <tbody>
                <tr  v-for="pokok in sortedPokok" :key="pokok.id"  >
                    <td>{{pokok.kta}}</td>
...
export default {
  ...
  methods: {
    ...
    loadSimpanan(){
        axios.get('http://localhost:3000/pokok').then(res => {
      ...
      add(){
      axios.post('http://localhost:3000/pokok', this.form).then(res => {
          this.load()
          this.form.penyetor = '',
          this.form.kta='',
          this.jenis_setoran='',
    ...
    update(form){ 
       return axios.put('http://localhost:3000/pokok' + form.id , {
    ...
    del(user){
      axios.delete('http://localhost:3000/pokok' + user.id).then(res =>{
          this.load()
